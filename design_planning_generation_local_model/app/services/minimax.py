...
def _call_minimax_api_raw_stream(
    system_prompt: str = "",
    user_prompt: str = "",
    temperature: float = 0.3,
    max_tokens: int = 16384,
    ...
    """流式调用本地Ollama API (qwen3.5:122b)，逐 chunk yield (kind, text)。

    与 _call_minimax_api_raw 的区别：stream=True，边生成边产出，kind ∈ {"thinking", "content"}。
    thinking 为模型思维链（qwen3 在正文前先流式输出思考过程），content 为正文。
    用于把「思考过程 + 正文」实时推送到前端的场景（文档生成/修改）。

    Yields:
        (kind, text) 元组，kind 为 "thinking" 或 "content"。
        失败时 yield 一次 ("content", "") 后结束（与 _call_minimax_api_raw 返回空串语义一致）。
    """
    base_url = os.getenv("OLLAMA_BASE_URL", "http://localhost:11435")
    model = os.getenv("OLLAMA_MODEL", "qwen3.5:122b")
    api_url = f"{base_url}/api/chat"
    ...
    payload = {
        "model": model,
        "messages": messages,
        # qwen3.5:122b 的 thinking 在流式模式下：先逐 chunk 输出 message.thinking，
        # 全部思考完再逐 chunk 输出 message.content。enable_thinking=False 时跳过思考阶段。
    ...
    for attempt in range(3):
        try:
            with requests.post(api_url, json=payload, stream=True, timeout=timeout) as response:
                response.raise_for_status()
    ...
    if last_error:
        print(f"[_call_minimax_api_raw_stream] 3次尝试后仍失败: {last_error}")
    yield ("content", "")
...
DOC_CHAPTERS = {
    ...
    "risk_management_plan": [
        {"id": "ch1", "name": "风险管理的范围", "query": "产品概述 产品描述 产品特性 适应症 风险管理的寿命周期阶段范围 贴敷式胰岛素泵"},
        {"id": "ch2", "name": "风险管理活动的人员和职责", "query": "风险管理活动参与人员 人员职责 风险管理团队 资格"},
        {"id": "ch3", "name": "风险管理活动及评审要求", "query": "风险管理活动定义 风险管理的评审要求 FMEA FTA 评审安排"},
        {"id": "ch4", "name": "风险管理的验证活动", "query": "风险管理验证 验证活动 验证方法 验证记录"},
        {"id": "ch5", "name": "产品生产及生产后风险管理", "query": "风险信息收集 更新风险分析 生产后信息 投诉 不良事件 文献"},
        {"id": "ch6", "name": "风险管理可接受性准则", "query": "严重度S 概率O 风险接收准则 评分标准 可接受矩阵"},
    ],
    ...
    "risk_management_report": [
        {"id": "ch1", "name": "风险管理范围", "query": "产品信息 适应症 工作原理 结构和组成 主要功能 贴敷式胰岛素泵 开环控制 记忆丝驱动"},
        {"id": "ch2", "name": "风险管理活动的人员和职责", "query": "相关人员 风险管理团队 人员职责 资格"},
        {"id": "ch3", "name": "风险分析和管理总结", "query": "危害识别 风险分析 风险评价 风险控制措施 验证 过量输注 输注不足 通信中断 软件失效"},
        {"id": "ch4", "name": "剩余风险评估", "query": "剩余风险 综合剩余风险评价 风险控制后 可接受性"},
        {"id": "ch5", "name": "风险/收益分析", "query": "风险收益分析 受益 风险比较 临床受益"},
        {"id": "ch6", "name": "上市后信息", "query": "生产后信息 投诉 不良事件 文献 上市后监测 信息收集"},
        {"id": "ch7", "name": "结论", "query": "风险管理评审结论 综合评价 批准"},
    ],
    "design_validation_report": [
        {"id": "ch1", "name": "概述", "query": "目的 范围 术语和定义 确认计划 样品选择 用户代表选择 用户的选择 设计确认"},
        {"id": "ch2", "name": "检查", "query": "检查 确认项目 操作步骤 接受准则 外观 功能检查 标签检查"},
        {"id": "ch3", "name": "模拟临床", "query": "模拟临床 确认项目 操作步骤 接受准则 模拟使用 贴敷 输注 App操作 报警"},
        {"id": "ch4", "name": "其他用户需求及验证方法和引用", "query": "其他用户需求 验证方法 引用 确认项目 验证报告引用"},
        {"id": "ch5", "name": "确认结论", "query": "确认结论 用户需求满足 综合评价 偏差处理"},
    ],
    ...
    "preliminary_risk_analysis": [
        {"id": "ch1", "name": "分析目的和范围", "query": "初步风险分析 目的 范围 适用标准 ISO 14971 附录C YY/T 1437-2023"},
        {"id": "ch2", "name": "产品安全性问题与特征清单", "query": "安全性问题 特征 问题 回答 危险分类 能量危害 生化危害 操作危害 信息危害 表格"},
        {"id": "ch3", "name": "危险分类汇总", "query": "能量危害 生化危害 操作危害 信息危害 分类归纳 输注过量 贴敷脱落 皮肤过敏 电磁干扰 数据错误"},
        {"id": "ch4", "name": "后续风险管理建议", "query": "风险管理计划 需要进一步分析的危害 设计阶段风险控制方向 优先级"},
    ],
    ...
    "cybersecurity_risk_analysis_matrix": [
        {"id": "ch1", "name": "总表说明和条目清单", "query": "网络安全 风险分析 管理总表 条目 资产 元素 BLE App 固件 数据 YY/T 1843-2022"},
        {"id": "ch2", "name": "威胁与初步风险评估", "query": "威胁分类 威胁行为描述 伤害 严重性 资产价值 影响程度 脆弱性 威胁赋值 被利用难易程度 风险定量水平 风险等级"},
        {"id": "ch3", "name": "防护措施与验证", "query": "防护措施描述 加密 认证 安全启动 验证结果 现行措施评估"},
        {"id": "ch4", "name": "剩余风险与可接受性", "query": "剩余风险 不能继续降低 可接受理由 风险受益分析"},
    ],
    ...
    "process_validation_plan": [
        {"id": "ch1", "name": "目的", "query": "工艺确认 目的 GMP 法规要求"},
        {"id": "ch2", "name": "范围", "query": "确认过程 确认设备 贴敷式胰岛素泵 生产工艺 装配 检验"},
        {"id": "ch3", "name": "参考文件", "query": "参考文件 引用标准 产品标准 工艺文件 质量体系文件"},
        {"id": "ch4", "name": "人员职责", "query": "人员职责 确认小组 资格 分工"},
        {"id": "ch5", "name": "确认时间和计划", "query": "确认时间 计划 进度安排"},
        {"id": "ch6", "name": "过程描述", "query": "材料信息 流程图 工艺流程 关键工序 mermaid"},
        {"id": "ch7", "name": "测量/测试设备和校准", "query": "测量设备 测试设备 校准 计量 校准有效期"},
        {"id": "ch8", "name": "设备操作维护保养", "query": "设备操作 维护保养 操作规程 保养计划"},
        {"id": "ch9", "name": "安装确认", "query": "安装确认 IQ 目的 确认方法和依据 确认类型 安装确认内容及可接受准则 异常情况处理 评价"},
        {"id": "ch10", "name": "运行确认", "query": "运行确认 OQ 目的 前提条件 确认方法和依据 运行确认内容 可接受准则 异常情况处理 评价"},
        {"id": "ch11", "name": "性能确认", "query": "性能确认 PQ 目的 前提条件 确认方法和依据 批次界定 抽样方案和可接受准则 异常情况处理 评价"},
        {"id": "ch12", "name": "再确认", "query": "再确认 再验证 触发条件 变更 周期"},
        {"id": "ch13", "name": "修订记录", "query": "修订记录 版本 修订日期 修订内容 修订人 审批"},
    ],
    ...
    "usability_validation_confirmation_report": [
        {"id": "ch1", "name": "可用性验证", "query": "测试项目名称 接收准则 测试方法和步骤 测试结果 测试结论 逐项验证 贴敷 剂量设定 报警响应 App操作"},
        {"id": "ch2", "name": "可用性确认计划", "query": "目的 测试方法 测试任务 测试参与人员 测试配置 测试环境配置 样品配置计划 测试程序 接受标准"},
        {"id": "ch3", "name": "测试结果分析", "query": "观察员数据记录总结 患者和监护人员使用困难总结 参与者评估数据记录总结 可用性确认结果 验证结论 附录"},
    ],
    "usability_specification": [
        {"id": "ch1", "name": "对可用性规范的输入", "query": "预期用途规范 预期用途 医疗器械应用规范摘要 任务要求 基本操作功能 经常使用的功能 和安全有关的功能 风险分析 用户特征 与安全相关的特征判定 预期危害和危险处境 有可能出错的事项 使用环境 现有类似器械已知的危害处境信息 导致的损害 用户接口构思的初始评审"},
        {"id": "ch2", "name": "可用性规范", "query": "总则 使用情景 和基本操作功能有关的用户动作 基本操作功能的用户接口要求 随附文件 时常出现的或安全性有关的使用情景的用户接口要求 确定基本操作功能是否易于被用户识别的要求"},
    ],
